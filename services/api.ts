...
export const saveVoters = async (voters: VoterRecord[]): Promise<ApiSaveResponse> => {
  try {
    // We use a regular POST here. Note: Google Apps Script POST with CORS 
    // often requires text/plain to avoid preflight issues in some environments,
    // or standard CORS if the script is deployed correctly.
    const response = await fetch(getBaseUrl(), {
      method: 'POST',
      body: JSON.stringify({ action: 'save', data: voters }),
    });
    
    // For GAS, we might not get a readable response with no-cors, 
    // but with standard fetch, we check success.
