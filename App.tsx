...
const App: React.FC = () => {
  ...
  const houseOptions = (booth && ward) ? houseMap[`${booth}_${ward}`] || [] : [];
        ...
        <div className="max-w-6xl mx-auto flex flex-col gap-6">
          ...
          <form onSubmit={handleSearch} className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2 bg-indigo-900/50 p-1.5 rounded-xl border border-indigo-800 shadow-inner">
              ...
              <select value={ward} onChange={e => { setWard(e.target.value); setHouse(''); }} className="appearance-none px-4 py-2 bg-transparent text-white rounded-lg outline-none w-32 font-medium cursor-pointer" required disabled={!booth || metaLoading}>
                <option value="" className="bg-indigo-950">वार्ड</option>
                {wardOptions.map(w => <option key={w} value={w} className="bg-indigo-950">{w}</option>)}
              </select>
              <div className="w-px h-6 bg-indigo-800" />
              <select value={house} onChange={e => setHouse(e.target.value)} className="appearance-none px-4 py-2 bg-transparent text-white rounded-lg outline-none w-32 font-medium cursor-pointer" required disabled={!ward || metaLoading}>
                <option value="" className="bg-indigo-950">मकान</option>
                {houseOptions.map(h => <option key={h} value={h} className="bg-indigo-950">{h}</option>)}
              </select>
            </div>
            <button type="submit" disabled={loading || !booth || !ward || !house} className="px-8 py-3 bg-white text-indigo-950 font-bold rounded-xl hover:bg-indigo-50 transition-all flex items-center gap-2 disabled:opacity-50">
