    ...
    <div className="bg-white rounded-[2.5rem] premium-shadow overflow-hidden border border-slate-50 hover:border-indigo-100 transition-all duration-500 flex flex-col h-full relative group">
      ...
      <div className="p-8 space-y-8 flex-grow">
        ...
        <div className="flex flex-col sm:flex-row gap-6 items-start">
          <div className="relative shrink-0 mx-auto sm:mx-0">
            ...
            >
              {voter.aadharPhoto ? (
                <>
                  <img src={voter.aadharPhoto} alt="Aadhar" className="w-full h-full object-contain" />
          ...
          <div className="flex-grow space-y-5 w-full">
            <div className="group/field">
              <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">निर्वाचक का नाम</label>
              <input 
                type="text" 
                value={voter.name} 
                onChange={(e) => handleFieldChange('name', e.target.value)} 
                className="w-full px-6 py-3.5 text-base font-bold text-slate-900 bg-slate-50 border border-slate-100 rounded-2xl focus:bg-white focus:border-indigo-200 outline-none transition-all" 
              />
            </div>
            <div className="group/field">
              <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">पिता/पति का नाम</label>
              <input 
                type="text" 
                value={voter.relationName} 
                onChange={(e) => handleFieldChange('relationName', e.target.value)} 
                className="w-full px-6 py-3.5 text-base font-medium text-slate-600 bg-slate-50 border border-slate-100 rounded-2xl focus:bg-white focus:border-indigo-200 outline-none transition-all" 
              />
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-4 gap-3">
          <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100 text-center">
            ...
            <select value={voter.gender} onChange={(e) => handleFieldChange('gender', e.target.value)} className="w-full text-sm font-black bg-transparent text-slate-800 appearance-none text-center"><option value="पु">पु</option><option value="म">म</option><option value="अन्य">अन्य</option></select>
          </div>
          <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100 text-center">
        ...
        <div className="bg-slate-50/50 p-6 rounded-[2rem] border border-slate-100 space-y-6">
          <div className="relative group/id">
            <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">आधार संख्या (12 अंक)</label>
            <input 
              type="text" 
              value={localAadhar}
              onChange={handleAadharChange}
              placeholder="0000 0000 0000"
              className="w-full px-6 py-4 border-2 border-white bg-white rounded-2xl focus:border-indigo-600 transition-all text-lg font-black tracking-[0.2em] outline-none shadow-sm text-center text-slate-900"
            />
          </div>
          ...
          <div className="flex items-center gap-4">
            <div className="flex-grow">
              ...
              <input 
                ...
                className="w-full px-6 py-4 bg-white border-2 border-white rounded-2xl focus:border-indigo-600 text-sm font-bold outline-none transition-all shadow-sm" 
              />
            </div>
            <div className="shrink-0 bg-indigo-600 px-6 py-4 rounded-[1.5rem] flex flex-col items-center justify-center shadow-xl border-b-4 border-indigo-800 active:translate-y-1 active:border-b-0 transition-all cursor-default">
      ...
      {isZoomed && voter.aadharPhoto && (
        <div className="fixed inset-0 z-[120] bg-slate-950/95 backdrop-blur-2xl flex flex-col items-center justify-center p-6" onClick={() => setIsZoomed(false)}>
          <img src={voter.aadharPhoto} alt="Aadhar" className="max-w-full max-h-[75vh] rounded-[3rem] shadow-2xl border-4 border-white/20 object-contain card-entry" />
          <div className="mt-10 bg-white/10 backdrop-blur-md px-10 py-4 rounded-full text-white font-black text-sm uppercase tracking-widest border border-white/10">आधार कार्ड दृश्य</div>
        </div>
      )}
    </div>
  );
